...
function hbase_classpath() {
  HBASE_CLASSPATH=""
  for jar in $(echo `$HBASE_HOME/bin/hbase classpath` | tr ":" "\n"); do
    case "$jar" in
      *jruby*)
        debug "Skipping JRuby jar: $jar"
      ;;
      *)
...
if [ -e "$HBASE_HOME/bin/hbase" ]; then
  hbase_classpath
  KITE_CLASSPATH="${KITE_CLASSPATH}:${HBASE_CLASSPATH}"
fi
debug "Using HBASE_CLASSPATH=${HBASE_CLASSPATH}"
...
WEB_APP_JAR=`echo target/ratings-app-*-jar-with-dependencies.jar`
debug "Using application JAR: ${WEB_APP_JAR}"
...
if [ -x "$HADOOP_COMMON_HOME/bin/hadoop" ]; then
  exec ${HADOOP_COMMON_HOME}/bin/hadoop jar "$WEB_APP_JAR" $flags "$@"
else
